...
def main():
    ...
    for i in range(args.iter_size):
        ...
        # stop mat_fi from printing a bunch of stuff
        # blockPrint()
        # insert errors
        out_mat = mat_fi(np.array(mat), seed=i, int_bits=b, frac_bits=0,
                        rep_conf=np.array([2**b]), q_type='unsigned')
        # enablePrint()
        ...
        for config_i, config in enumerate(configs):
            ...
            # mat = np.array([mlc.enc(j) for j in in_vals])
    ...
    if args.plot:
        plt.hist(errs_perc, bins=[x/20 for x in range(0,21)], align='mid')
        plt.title(f'Distribution of errors for {c} {b}-bit cells, {args.arr_size} numbers for {args.iter_size} iterations')
