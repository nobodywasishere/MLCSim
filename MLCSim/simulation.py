    ...
    if b == 2:
        ...
        if c == 2:
            ...
            configs = [
                ...
                # [[5, 6, 9, 14], [4, 7, 8, 15], [2, 3, 10, 13], [0, 1, 11, 12]]
            ]
    return configs
...
def __main():
    ...
    for i in range(args.iter_size):
        ...
        mat = [
            [random.randint(0, 2 ** (b) - 1) for _ in range(c)]
            for j in range(args.arr_size)
        ]
    ...
    print(
        f"{len(configs[0])} {len(configs[0][0])}-bit cells, {total_count} numbers tested:"
    )
